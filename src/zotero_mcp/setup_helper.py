...
def update_claude_config(config_path, zotero_mcp_path, semantic_config=None):
    ...
    # Create environment settings for the local desktop helper.
    env_settings = {
        "ZOTERO_LOCAL": "true",
        "ZOTERO_LIBRARY_ID": "0",
...
def _write_standalone_config(semantic_config: dict, no_claude: bool = False) -> Path:
    ...
    # Provide a helper env section for local desktop clients.
    client_env = {
        "ZOTERO_LOCAL": "true",
        "ZOTERO_LIBRARY_ID": "0",
        "ZOTERO_LIBRARY_TYPE": "user",
...
def main(cli_args=None):
    ...
    try:
        if args.no_claude:
            ...
            print("\nSetup complete (standalone/local mode)!")
