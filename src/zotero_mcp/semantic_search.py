...
class ZoteroSemanticSearch:
    ...
    def _get_items_from_source(self, limit: int | None = None, extract_fulltext: bool = False, chroma_client: ChromaClient | None = None, force_rebuild: bool = False) -> list[dict[str, Any]]:
        """
        Get items from the local Zotero database.
    ...
    def _get_items_from_api(self, limit: int | None = None) -> list[dict[str, Any]]:
        """
        Legacy fallback for non-local environments.
        ...
        while True:
            ...
            try:
                items = zotero_client.items(**batch_params)
            except Exception as e:
    ...
    def search(self,
        ...
        """
        try:
            self._ensure_database_ready_for_search()
            collection_count = self.chroma_client.get_collection_info().get("count", 0)
            ...
            query_limit = min(max(limit * 3, limit), collection_count)
    ...
    def _ensure_database_ready_for_search(self) -> dict[str, Any] | None:
        """Refresh the semantic index before search when the current state is clearly stale."""
        collection_info = self.chroma_client.get_collection_info()
        collection_count = int(collection_info.get("count", 0) or 0)

        local_count: int | None = None
        try:
            with LocalZoteroReader(db_path=self.db_path) as reader:
                local_count = int(reader.get_item_count())
        except Exception as exc:
            logger.warning("Could not determine local Zotero item count before semantic search: %s", exc)

        if collection_count <= 0:
            logger.info("Semantic index is empty; rebuilding before search")
            return self.update_database(force_full_rebuild=True, extract_fulltext=False)

        if local_count is not None and local_count != collection_count:
            logger.info(
                "Semantic index count (%s) does not match local Zotero item count (%s); rebuilding before search",
                collection_count,
                local_count,
            )
            return self.update_database(force_full_rebuild=True, extract_fulltext=False)
        ...
        if self.should_update_database():
            logger.info("Semantic index marked stale by update policy; refreshing before search")
            return self.update_database(force_full_rebuild=False, extract_fulltext=False)
    ...
    def _enrich_search_results(self, chroma_results: dict[str, Any], query: str) -> list[dict[str, Any]]:
        ...
        for i, item_key in enumerate(ids):
            try:
                with LocalZoteroReader(db_path=self.db_path) as reader:
                    zotero_item = reader.get_item_details_by_key(item_key)
                if not zotero_item:
                    logger.info(
                        "Removing stale semantic index entry for deleted item %s", item_key
                    )
